...
var ry = /y+/, 
	...
	rMdhmsG = /[Mdhms]+/g,
	rDigitsG = /\d+/g,
	rLastDigitsClassifier = /^(last|past|next)([0-9]*)([dD]ays|[wW]eeks|[mM]onths|[qQ]uarters|[yY]ears|[cC]enturies)$/,
	...
	Object.assign(Date, {
		...
		/**
		 * 解析时间跨度词，返回起始和结束两个时间格式字符串的数组
		 * @param {string} rangeWord
		 * @param {string} format
		 * @returns {Array.<string>}
		 */
		parse2DateFormatsByRangeWord: function(rangeWord, format) {
			var dates = parse2DatesByRangeWord(rangeWord);
			format || ( format = Date.FORMAT );
			return [
		...
		/**
		 * 解析时间跨度词，返回起始和结束两个时间对象的数组
		 * @param {string} rangeWord
		 * @returns {Array.<Date>}
		 */
		parse2DateObjectsByRangeWord: function(rangeWord) {
			return parse2DatesByRangeWord(rangeWord);
		},
	 ...
	 */
	function parse2DatesByRangeWord(rangeWord) {
		...
		switch (rangeWord) {
			...
			default:
				...
				if (rLastDigitsClassifier.test(rangeWord) && (number = parseInt(RegExp.$2)) > 0) {
